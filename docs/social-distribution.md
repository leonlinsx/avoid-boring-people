...
New articles can use all eligible destinations. Evergreen distribution is limited to Bluesky, Mastodon, Farcaster, Nostr, and Threads once Threads is promoted; DEV is never recycled and X is out of the defaults while its API credits are depleted.
...
- `THREADS_USER_ID` and `THREADS_ACCESS_TOKEN` (a long-lived Threads API token for the `avoidboringpeople` profile)
...
LinkedIn, Reddit, Threads, and Weibo remain out of the defaults until their
...
Threads publishes through the Graph API's two-step container flow: create a container at `POST /v1.0/{user-id}/threads`, then publish it at `POST /v1.0/{user-id}/threads_publish`. It is implemented but stays out of `DEFAULT_PLATFORMS` until a live run verifies the credentials and the publish flow, so today it only runs through an explicit `PLATFORM=threads`.
...
Verify the integration before trusting it:
...
Also run `DRY_RUN=true POST_MODE=thread PLATFORM=threads python -m scripts.automation.auto_post` locally first; the dry run prints the exact text of both posts and is preferred for reviewing the rendered voice. That run summarizes through the local TextRank stub; add `USE_LLM=true TEST_API=true` to review the DeepSeek summary that production actually uses, because `DRY_RUN` alone makes the summarizer return mock points.

To promote Threads, add it to `DEFAULT_PLATFORMS` in `scripts/automation/routing.py` and update this document.

Long-lived Threads tokens expire after 60 days and are refreshed with `GET /refresh_access_token?grant_type=th_refresh_token`. Refresh out of band and rotate the secret; the adapter never refreshes on its own, so an expired token surfaces as a `401` that is reported as permanent rather than retried. The token travels in an `Authorization: Bearer` header, so it never appears in a request URL or in an error message.
