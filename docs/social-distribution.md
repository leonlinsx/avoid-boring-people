...
- `INSTAGRAM_USER_ID`, `INSTAGRAM_ACCESS_TOKEN`, `INSTAGRAM_MEDIA_UPLOAD_URL`, and `INSTAGRAM_MEDIA_UPLOAD_SECRET` for a manual Instagram run; `INSTAGRAM_MEDIA_BASE_URL` is the alternative to the upload endpoint when the slides already live somewhere Meta can fetch. See [Instagram](#instagram-implemented-manual-only)
...
Remaining risk: no carousel has been published yet. The first manual run did prove the parts before the first Instagram call: the storyboard rendered, every slide uploaded to the public Blob store through the OIDC endpoint, and Meta's host rejected the credentials with `401 Invalid OAuth access token - Cannot parse access token` because the adapter was calling `graph.facebook.com` with an Instagram Login token. That host mismatch is fixed, so the container flow and the publish call are still unexercised live; its media id, permalink, and outcome belong here afterwards.

### Credentials

`INSTAGRAM_USER_ID` is the Instagram professional account id, and `INSTAGRAM_ACCESS_TOKEN` is an **Instagram User access token** minted by the app's Instagram Login setup, with `instagram_business_basic` and `instagram_business_content_publish` granted. Both are used against `https://graph.instagram.com/v26.0`, which is the host for Instagram Login; the token travels in an `Authorization: Bearer` header, so it never appears in a request URL or in an error message.
...
A live run is a manual `Social New Article` dispatch with `platforms=instagram` and a `post_id`, or a local `PLATFORM=instagram python -m scripts.automation.auto_post`. It renders the slides, POSTs each JPEG to the upload endpoint, and hands the returned public HTTPS URLs to the publisher, which verifies them before creating the first container; the final Instagram media id is then recorded in `posted.json` like any other channel. The workflow installs Node and runs `npm ci --omit=dev` with browser downloads disabled, but only when `platforms` contains `instagram`, so the renderer is present exactly when it is needed and the unattended default path stays unchanged. The Instagram credentials and the upload endpoint values are read by the manual workflow alone: the unattended `Social Evergreen` schedule has no Instagram or Blob configuration and must not gain any. Publishing to Instagram never happens from a push, a build, or an unattended evergreen cycle.
...
7. Do the first live publish manually with `platforms=instagram` (lowercase, as the workflow gate that installs the renderer matches that literal value) and a `post_id`, then record the media id, permalink, and outcome here before adding Instagram to `DEFAULT_PLATFORMS`.
