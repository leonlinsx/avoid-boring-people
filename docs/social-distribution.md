...
Hashtags come only from sanitized article metadata, never from the model: the X/Bluesky thread root, the Mastodon status, the Farcaster cast, and the Threads main post each append up to three `#tags` when everything fits, dropping them before trimming content. Mastodon prefers two whole supporting points over one within its 500-character budget; the trim fallback stays tagless.
