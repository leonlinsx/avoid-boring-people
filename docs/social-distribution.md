...
New articles are distributed as soon as the push-triggered run sees them live in the deployed search index. The back catalog is served by `social-evergreen.yml`, one article per run, on Tuesday and Thursday at 09:37 and Sunday at 17:37 `America/New_York`. GitHub cron is UTC-only and has no DST support, so both offsets of each window are scheduled (13:37/14:37 and 21:37/22:37 UTC) and a `gate` job keeps the run whose `github.event.schedule` equals the DST-correct UTC time of today's window. Reading which cron fired, rather than the wall clock, means a run delayed under load still serves the right window and the wrong offset never publishes; the gate announces a skip through a run notice, and a manual dispatch bypasses it. A scheduled run that arrives without a cron identifier, or a runner whose tz data cannot resolve `America/New_York`, fails the gate rather than guessing, so the only way a window is lost without a signal is when GitHub never fires the cron at all. The per-platform 60/90-day evergreen cooldowns in `scripts/automation/state_manager.py` are unchanged.
...
4. Note the new token's mint date where you track secrets and refresh again before day 60. A lapse otherwise surfaces as the probe's `::error::` failure at 08:00 UTC, ahead of the evergreen windows — rotate immediately, since an expired token fails the publish run as a permanent `401`, not a retry.
...
Container processing is asynchronous on Meta's side and the publish path can lag behind container creation, so the adapter waits for `GET /{container-id}?fields=status` to report `FINISHED` before publishing and retries `threads_publish` when Meta answers as if the media did not exist. That answer is HTTP 400 with `code 24`, `subcode 4279009`, and `error_user_title` `Media Not Found`, and it is returned for a container created moments earlier as well as for one that never existed; `is_transient` is `false` in the payload even though the identical call succeeds a few seconds later, so the retry is decided by the code rather than by the HTTP status. Up to five attempts, three seconds apart, reuse the same `creation_id`, because that error means nothing was published and repeating it cannot duplicate a post. Nothing is retried blindly: before each retry Meta is asked for the container status, and a container that reports `PUBLISHED` is not retried at all, because that status means the post is live and a second publish could duplicate it. A container that reports `EXPIRED` or `ERROR` fails immediately instead, and the next run builds a fresh one. Both the wait and the retry are advisory about ordering only: `threads_publish` stays the single authority on whether a post exists, and the wait can never be why a publish is skipped.

Failures name the call that failed and carry Meta's own codes, for example `❌ Threads API error 400 (publish container 12345): The requested resource does not exist [code 24, subcode 4279009, Media Not Found]`. The generic message alone cannot distinguish a container that is not readable yet from one that was deleted, which is why the code, subcode, and user-facing title are logged.
...
3. Confirm the main post and the link reply appear on the profile and that `posted.json` records the root media id. A container that needed the propagation retry is visible in the log as `⚠️ Threads container ... is not readable on the publish path yet (attempt n/5)`, so a repeated race is distinguishable from a failure that needs a code change.
...
- Threads has no idempotency key. Only a failure that is known to have published nothing is retried: `code 24` is repeated because Meta reports the container as missing, while a publish that returns 200 without a media id, or that fails after the root post is already live, is raised as a non-transient failure instead of being retried. A retry loop can therefore never turn a lost response into a duplicate post. If a run ends with `Media Not Found` after every attempt, the container is genuinely gone rather than merely slow, so re-running is safe and builds a fresh container. If the log instead reports that the container `reports PUBLISHED after a failed publish`, the post is live: add its `posted.json` entry rather than re-running, because an unrecorded post is published again by the next evergreen cycle.
