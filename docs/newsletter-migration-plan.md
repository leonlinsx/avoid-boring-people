...
Every mail includes the visible From `Leon Lin <newsletter@leonlins.com>`, Reply-To `contact@leonlins.com`, a human-readable unsubscribe link, compliant footer, privacy-policy link, and `List-Unsubscribe` plus `List-Unsubscribe-Post: List-Unsubscribe=One-Click` headers. Production sending is blocked until a real compliant postal address is configured; never hard-code a guessed address.
