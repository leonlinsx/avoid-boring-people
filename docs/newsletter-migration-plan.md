...
- `GET /api/newsletter/confirm`: renders a non-mutating page with a confirm button, so mailbox scanners and link prefetchers cannot confirm on the reader's behalf. It renders that page for whatever token it is given and only reports an unknown or superseded token as invalid after the POST, because validating on the GET would mean a database read on a scanner-reachable request. `POST` validates the one-time confirmation token and transitions a `pending` or `unsubscribed` subscriber to active.
...
Only `pending -> active` is an automatic activation transition, and `unsubscribed -> active` happens only when the reader asks for the newsletter again and confirms the fresh token: a form post against an `unsubscribed` row issues a new confirmation token and leaves the row suppressed, and the confirmation link is the only thing that reactivates it. `bounced` and `complained` are deliverability facts rather than preferences, so those rows are never reopened by a signup form — a request against one returns the same generic response as an `active` row and issues no token. `active` may move to `unsubscribed`, `bounced`, or `complained`; automated jobs and imports may never reactivate any suppressed status. A resubscription keeps its first `confirmed_at` and its original attribution, so the report reads a returning reader rather than a new subscriber, and a later cancellation records its own `unsubscribed_at`. A cancellation also clears any outstanding confirmation token, so a confirmation email the reader never clicked cannot reopen a row they cancelled afterwards. Tokens are cryptographically random and only their hashes are stored. Raw tokens must not be logged.
...
### Phase 4 companion — failure alerting and explicit resubscription (added 2026-09-18)

Make a failing newsletter job visible without adding monitoring infrastructure. `src/lib/newsletter/alerting.ts` emits one greppable `newsletter_alert` log line for exactly three kinds — `signup_pipeline_failure`, `ses_event_ingestion_failure`, and `analytics_job_failure` — and copies only `reason` and `errorName`, so no email address, token, or request body can reach it. `npm run newsletter:analytics -- --check` fails the process on a bounce-rate or complaint-rate breach, or when sends past the one-hour grace period have no correlated SES event at all, and the scheduled `newsletter-health.yml` workflow runs that weekly, so a failed run is the notification. Ordinary signal, such as "no campaign was sent", stays report-only rather than turning the weekly run red for an irregular publishing cadence. The subscription lifecycle also gained the explicit resubscription described under data and consent: an `unsubscribed` reader can ask again through the normal double opt-in, while `bounced` and `complained` rows stay closed.
...
- Subscription lifecycle: generic responses, honeypot/rate limits, token hashing, confirmation, idempotent unsubscribe, and forbidden transitions — including that a resubscription request issues a token without changing status, that only a matching token confirms it, and that `active`, `bounced`, and `complained` rows are never rewritten by a request. The real SQL is exercised by `tests/newsletter-lifecycle-postgres.ts`.
...
- Failure alerting: only the three failure kinds emit an alert, the alert carries no field outside `reason` and `errorName`, unauthenticated SNS traffic and malformed signup bodies stay silent, and `--check` exits non-zero exactly when the window holds a deliverability or ingestion failure.
