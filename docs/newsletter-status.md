...
- AWS identity readiness is healthy: `leonlins.com` is verified, DKIM succeeds with 2048-bit keys, `mail.leonlins.com` MAIL FROM succeeds, and account suppression covers bounces and complaints. Authentication is reported passing end to end (see Phase 4): SPF, DKIM, and DMARC all pass on production mail. DMARC policy remains monitoring-only (`p=none`), so a policy change is still outstanding even though alignment already passes.
...
- **Next controlled gate:** Phase 4 validation with test accounts only. The local-sender IAM `contact@leonlins.com` recipient restriction remains locked until each broader cohort is explicitly approved; the Substack import list is never a recipient until you say so.
- Phase 4 test validation 2026-09-10: `[TEST]` article mail accepted by production SES and inbox-placed with intact rendering and footer links; simulator bounce/complaint sends accepted after a temporary IAM widening (since reverted), with delivery + terminal-event receipts recorded against zero subscribers. IAM denials for non-allowlisted recipients confirmed working in both directions.
...
- Added isolated on-demand routes under `/api/newsletter/*` for a generic-response subscription request, one-time confirmation, and idempotent unsubscribe. The unsubscribe GET only renders a confirmation page, preventing link scanners from changing state; both the visible form and originless RFC one-click request mutate only via POST with the hashed bearer token. Suppressed subscribers remain suppressed. `SubscribeForm.astro` remains untouched; `src/pages/api/subscribe.ts` behavior is unchanged, with a one-line `prerender = false` flag added later so the fallback route actually deploys on the static-default site.
...
- Added the public `/privacy/` notice, published at `/privacy/` and linked from newsletter emails but intentionally omitted from the primary navigation and site footer. It accurately distinguishes the current Substack sign-up path from the planned Neon/SES migration and documents newsletter data, providers, unsubscribe/suppression, and contact choices.
...
- The confirmation email is now a normal transactional message with both HTML and plain-text bodies: subject `Confirm your subscription to Avoid Boring People`, From `Leon Lin <newsletter@leonlins.com>`, Reply-To `contact@leonlins.com`, a direct `https://leonlins.com/api/newsletter/confirm?token=...` destination rendered as an email-safe button in HTML and printed in full in the plain-text alternative. No tracking, pixels, shorteners, redirect domains, or remote images. The SES configuration set and all delivery safety gates are retained.
...
1. Raise the DMARC policy above monitoring-only. Alignment already passes per the reported 2026-09-13 Gmail check, so the remaining step is a DNS change at the registrar/DNS provider the repository does not control. Do not change DNS or apply `quarantine`/`reject` without explicit approval, and re-check alignment after any DNS or SES edit.
