...
- **Implementation remaining in Phase 3:** production campaign preparation/sender CLI, batching/rate/quota checks, crash-safe resume behavior at the CLI boundary, and an applied importer (only dry-run inspection exists).
- **Next controlled gate:** run SES mailbox-simulator bounce and complaint tests, then verify authenticated receipts. This sends two synthetic messages to AWS-operated simulator addresses and requires action-time approval.
