...
2026-09-09 controlled-test update (supersedes earlier live-deployment notes): applied approved SNS `AllowNewsletterSesEvents` policy restricted to SES, account 079415246848 and configuration set `my-first-configuration-set`. Created the `newsletter-ses-events` destination for DELIVERY/BOUNCE/COMPLAINT. Exactly one local marked test to contact@leonlins.com was accepted (SES message ID `010f01a0845923b7-94eec99c-7483-4ef7-a4a7-3146f500ef91-000000`). No subscriber list read or sending-permission expansion. End-to-end ingestion FAILED verification: zero recent database receipts; the live domain now points to newer main deployment `dpl_7TKB8tejDEBzXoska3MkWJtX7Xf9`, and invalid POST to the SNS route returns 404. SNS metrics showed two published/delivered notifications but do not prove application ingestion. Disabled the new SES event destination and verified `Enabled=false`; retained the scoped policy and confirmed subscription. Preserve the newer main site changes. Next: integrate newsletter fixes into the current production branch, deploy and verify the handler, then re-enable events and repeat controlled validation. No bounce/complaint simulator tests performed. Substack remains untouched.
