...
| --- | --- | --- | --- |
| `ci.yml` | Push to `main`, pull request, manual dispatch | Runs `npm test`, `python -m pytest tests -q`, and `npm run build` in three independent jobs | Fails the run on any test or build failure; no secrets needed |
