  ...
  return execFileSync(
    ...
    ],
    {
      ...
      env: {
        ...
        PGOPTIONS: '-c search_path=newsletter_event_test,public',
      },
    },
  ).trim();
}
...
const db = (async (parts: TemplateStringsArray, ...values: unknown[]) => {
  const query = parts.reduce(
    (query, part, index) =>
      query + part + (index < values.length ? literal(values[index]) : ''),
    '',
  );
...
const bounce = (id: string) =>
  event(id, 'Bounce', {
    bounce: {
      ...
      bouncedRecipients: [{ emailAddress: ' PERSON@example.com ' }],
    },
  });
const delivery = (id: string) =>
  event(id, 'Delivery', { delivery: { timestamp: '2026-09-08T11:00:00Z' } });
const state = () =>
  sql(
    "SELECT s.status || ':' || r.status FROM subscribers s JOIN campaign_recipients r ON r.subscriber_id=s.id",
  );
sql('CREATE SCHEMA newsletter_event_test');
try {
  sql(
    readFileSync(
      new URL('../migrations/newsletter/001_initial.sql', import.meta.url),
      'utf8',
    ),
  );
  sql(
    readFileSync(
      new URL(
        '../migrations/newsletter/002_event_and_campaign_state.sql',
        import.meta.url,
      ),
      'utf8',
    ),
  ...
  sql(
    readFileSync(
      new URL(
        '../migrations/newsletter/003_production_send_safety.sql',
        import.meta.url,
      ),
      'utf8',
    ),
  ...
  sql(
    "ALTER TABLE subscribers ADD CONSTRAINT forced_failure CHECK (status <> 'bounced')",
  );
  ...
  await recordSesEvent(bounce('retry'), db);
  assert.equal(
    sql('SELECT row_to_json(r) FROM campaign_recipients r'),
    snapshot,
  );
  await recordSesEvent(delivery('late-delivery'), db);
  assert.equal(state(), 'bounced:bounced');
  assert.equal(
    sql('SELECT delivered_at IS NOT NULL FROM campaign_recipients'),
    't',
  );
  // Synthetic fixture reset, confined to this disposable schema.
  sql(
    "UPDATE subscribers SET status='active'; UPDATE campaign_recipients SET status='sent'",
  );
  ...
  await recordSesEvent(
    event('complaint', 'Complaint', {
      complaint: {
        complainedRecipients: [{ emailAddress: 'person@example.com' }],
      },
    }),
    db,
  );
  ...
  await Promise.all(
    Array.from({ length: 5 }, () =>
      recordSesEvent(bounce('concurrent-retry'), db),
    ),
  );
  assert.equal(
    sql(
      "SELECT count(*) FROM newsletter_event_receipts WHERE event_id='concurrent-retry'",
    ),
    '1',
  );
  sql(
  ...
  await Promise.all([
    recordSesEvent(bounce('concurrent-bounce'), db),
    recordSesEvent(delivery('concurrent-delivery'), db),
  ]);
  assert.equal(state(), 'bounced:bounced');
  sql(
    "UPDATE subscribers SET status='active'; UPDATE campaign_recipients SET status='sending', provider_message_id=NULL",
  );
  await recordSesEvent(
    event('early-complaint', 'Complaint', {
      complaint: { timestamp: '2026-09-08T12:30:00Z' },
    }),
    db,
  );
  assert.equal(state(), 'active:sending');
  sql(
    "UPDATE campaign_recipients SET status='sent', provider_message_id='test-message'",
  );
  await reconcileSesMessage('test-message', db);
  assert.equal(state(), 'complained:complained');
  console.log(
    'Newsletter Postgres integration checks passed (rollback, retry, concurrent replay/events, late/early events, redaction, suppression).',
  );
} finally {
