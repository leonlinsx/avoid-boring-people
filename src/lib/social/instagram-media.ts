...
export const BLOB_PATH_PATTERN =
  /^instagram\/[A-Za-z0-9_-]{1,96}\/[0-9a-f]{12}\/slide-\d{2}\.jpg$/;
...
export async function handleInstagramMediaUpload(
  ...
  if (secret === '') {
    console.error(
      'Instagram media upload rejected: INSTAGRAM_MEDIA_UPLOAD_SECRET is not configured.',
    );
  ...
  if (contentType !== 'image/jpeg')
    return jsonResponse({ error: 'expected an image/jpeg body' }, 415);

  const pathname = (request.headers.get(BLOB_PATH_HEADER) ?? '').trim();
  if (!BLOB_PATH_PATTERN.test(pathname)) {
    console.error(
      'Instagram media upload rejected: object path is not a carousel slide path.',
    );
  ...
  try {
    ...
    if (url === '' || !url.startsWith('https://')) {
      console.error(
        'Instagram media upload failed: the store did not return an HTTPS URL.',
      );
      return jsonResponse(
        { error: 'the store did not return an HTTPS URL' },
        502,
      );
    }
    if (returnedPath !== '' && returnedPath !== pathname) {
      console.error(
        'Instagram media upload failed: the store stored a different pathname.',
      );
      return jsonResponse(
        { error: 'the store stored a different pathname' },
        502,
      );
    }
    console.info('Instagram carousel slide stored.', {
