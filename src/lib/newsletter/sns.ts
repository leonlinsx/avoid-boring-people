  ...
  return new Promise((resolve, reject) => {
    const request = get(url, { signal }, (response) => {
      ...
      response.on('data', (chunk) => {
        ...
        if (bytes > 65_536)
          request.destroy(new Error('SNS HTTPS response exceeds limit.'));
        else body += chunk;
      });
...
function text(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value)
    throw new Error(`SNS message is missing ${field}.`);
  return value;
}
...
export function parseSnsEnvelope(value: unknown): SnsEnvelope {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('SNS body must be an object.');
  ...
  const envelope: SnsEnvelope = {
    ...
    SignatureVersion: text(
      ...
      'SignatureVersion',
    ) as SnsEnvelope['SignatureVersion'],
  ...
  if (Number.isNaN(Date.parse(envelope.Timestamp)))
    throw new Error('SNS timestamp is invalid.');
  if (
    envelope.Type === 'SubscriptionConfirmation' &&
    (!envelope.SubscribeURL || !envelope.Token)
  ) {
...
function snsUrl(value: string, region: string, certificate: boolean): URL {
  ...
  if (
    ...
    throw new Error('SNS URL is not from the expected AWS SNS endpoint.');
  if (
    certificate &&
...
export async function verifySnsEnvelope(
  ...
  if (!expectedTopicArn || message.TopicArn !== expectedTopicArn)
    throw new Error('SNS TopicArn is not expected.');
  ...
  const certificate = fetchImpl
    ? await fetchImpl(certificateUrl, { signal, redirect: 'error' }).then(
        async (response) => {
          if (!response.ok)
            throw new Error('SNS signing certificate could not be retrieved.');
  ...
  if (!verifier.verify(certificate, message.Signature, 'base64'))
    throw new Error('SNS signature is invalid.');
}
...
export async function confirmSnsSubscription(
  ...
  if (
    ...
    throw new Error(
      'SNS subscription confirmation URL is not for the expected topic.',
    );
  }
  const signal = deadline
    ? AbortSignal.any([deadline, AbortSignal.timeout(4_000)])
