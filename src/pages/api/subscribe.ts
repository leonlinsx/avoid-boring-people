...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!email || !/^\S+@\S+\.\S+$/.test(email)) {
      return new Response(JSON.stringify({ ok: false, message: "Invalid email" }), { status: 400 });
    }
    ...
    return new Response(JSON.stringify({ ok: false, status: res.status, message: text }), { status: 502 });
  } catch (err: any) {
    return new Response(JSON.stringify({ ok: false, message: err?.message || "Request failed" }), { status: 500 });
  }
};
