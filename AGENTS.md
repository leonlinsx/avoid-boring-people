...
- Contact notes are first-party and minimal: three fields on `/about`, `/now`, and direct-link `/contact`, one `contact_submissions` row per note (`migrations/contact/001_initial.sql`), one SES notification to `contact@leonlins.com` with no SES configuration set attached, and an optional server-to-server handoff (`src/lib/contact/lin-check.ts`, `docs/contact.md`) that reports an inbound and never creates a Lin Check Person, Organization, or relationship. Failures emit one `contact_alert` line (`src/lib/contact/alerting.ts`) rather than retrying, the SES send is bounded by `NOTIFICATION_TIMEOUT_MS` inside the browser's own wait, and `notified_at` is written before the optional handoff runs so a delivered note is never recorded as unfinished. There is no rate limit beyond Turnstile, the honeypot, the request-size cap, and the same-origin check.
- Lin Scout (`scripts/scout/`, `docs/scout.md`) is a separate internal discovery tool: it reads the distribution job's `search-index.json`, searches Hacker News and Bluesky, deterministically gates candidates, asks the existing summarizer's provider for one verdict per survivor, and reports at most a handful of opportunities with a drafted reply in the workflow step summary. It has no publisher and no database; its only state is the committed `scout-state.json`, and `.github/workflows/scout.yml` runs it daily. An optional, off-by-default Jev shadow evaluation (`scripts/scout/jev.py`, enabled only by `SCOUT_JEV_SHADOW` plus `TYPESAFE_API_KEY`) records what TypeSafe's Jev answers about the same candidates into an ignored local `scout-shadow.jsonl`; nothing reads it, no workflow enables it, and deleting it cannot change a run.
...
- A contact note stays a note: it is never scored or qualified, never a relationship record, never a newsletter subscription, and never handled by an automated funnel; no IP address, browser token, or message body may reach a log line.
- The contact form is an invitation, not navigation: `/about`, `/now`, and `/contact` offer it, and no nav or footer links to it. A missing `PUBLIC_TURNSTILE_SITE_KEY` removes the form and leaves the email address; a missing `DATABASE_URL` or `AWS_REGION` still renders the form, which then fails at submit with a message naming the email address.
...
| Scout shadow experiment | `python -m scripts.scout jev` (read-only comparison of `scout-shadow.jsonl`); needs `SCOUT_JEV_SHADOW=1` and `TYPESAFE_API_KEY` (plus `python -m pip install typesafe-sdk`) before a run records anything, and is deliberately absent from CI. |
...
- Contact changes must keep the form an invitation: three fields, no scoring or qualification, no relationship record, no newsletter signup inside the form, no CRM, no automation, no navigation entry, and no second place to read notes.
