...
if (!campaignId || !process.argv.includes('--confirm-production')) {
  throw new Error(
    'Usage: npm run newsletter:send -- <campaign-id> --expect-recipients <count> --confirm-production',
  );
}
...
if (campaigns.length !== 1)
  throw new Error('Campaign was not found. No email was sent.');
...
if (campaign.status !== 'draft')
  throw new Error(
    `Campaign status is ${campaign.status}; manual reconciliation is required and no email was sent.`,
  );
if (campaign.indeterminate_count > 0) {
  throw new Error(
    'Campaign has an indeterminate sending recipient. Reconcile it manually; no email was sent.',
  );
}
...
const pending =
  await db`SELECT count(*)::int AS count FROM campaign_recipients WHERE campaign_id = ${campaignId} AND status = 'pending'`;
...
const started =
  await db`UPDATE campaigns SET status = 'sending', started_at = COALESCE(started_at, now())
  WHERE id = ${campaignId} AND status = 'draft' RETURNING id`;
if (started.length !== 1)
  throw new Error('Another process changed the campaign; no email was sent.');
...
while (true) {
  ...
  const token = subscriberUnsubscribeToken(
    subscriberId,
  ...
  try {
      ...
      new SendEmailCommand({
        ...
        Content: {
          Simple: {
            ...
            Headers: Object.entries(rendered.headers).map(([Name, Value]) => ({
              Name,
              Value,
            })),
          },
    ...
    if (!result.MessageId)
      throw new Error(
        'SES accepted the request without returning a message id.',
      );
    messageId = result.MessageId;
  } catch (error) {
    console.error(
      'Send outcome is indeterminate; the recipient remains in sending state and will not be retried automatically.',
    );
    throw error;
  }
  ...
  try {
    const recorded =
      await db`UPDATE campaign_recipients SET status = 'sent', provider_message_id = ${messageId}, sent_at = now()
      WHERE campaign_id = ${campaignId} AND subscriber_id = ${subscriberId} AND status = 'sending'
      RETURNING subscriber_id`;
    if (recorded.length !== 1)
      throw new Error('The claimed campaign recipient could not be finalized.');
    await reconcileSesMessage(messageId, db);
  } catch (error) {
    console.error(
      'SES accepted the message, but database finalization is incomplete. Do not rerun; reconcile manually.',
    );
    throw error;
  }
  sent += 1;
...
if (
  Number(remaining[0]?.pending ?? 0) === 0 &&
  Number(remaining[0]?.sending ?? 0) === 0
) {
  await db`UPDATE campaigns SET status = 'completed', completed_at = now() WHERE id = ${campaignId} AND status = 'sending'`;
}
console.log(
  JSON.stringify(
    { campaignId, sent, skipped, remaining: remaining[0] },
    null,
    2,
  ),
);
