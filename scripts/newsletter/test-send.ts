...
if (!articleId || !recipient || confirmation !== '--confirm-test') {
  throw new Error(
    'Usage: npm run newsletter:test -- <article-id> <allowlisted-recipient> --confirm-test',
  );
}

const testRecipient = assertAllowedTestRecipient(
  recipient,
...
if (!configurationSet || !process.env.AWS_REGION) {
  throw new Error(
    'Test send requires SES_CONFIGURATION_SET and AWS_REGION. No email was sent.',
  );
}
...
if (!title)
  throw new Error('The selected article has no title. No email was sent.');
const unsubscribeUrl = new URL(
  '/api/newsletter/unsubscribe?token=test-only-not-a-subscriber',
  NEWSLETTER_PRIVACY_URL,
).toString();
...
const result = await new SESv2Client({ region: process.env.AWS_REGION }).send(
...
console.log(
  `Test email accepted by SES for ${testRecipient}; message id: ${result.MessageId ?? 'unavailable'}`,
);
